...
print.DiscNaiveBayesianModel <- function (self)
{
    ...
    cat ( "\tsize of Gaussian filter: ", as.character(dim(self$v.G)), "\n" )
    #cat ( "\tG: ", self$v.G, "\n" )
    ...
    cat ( "\tfreq0(Background prior): ", self$v.model$freq0, "\n" )
    cat ( "\tfreq1(Foreground prior): ", self$v.model$freq1, "\n" )
    cat ( "\tforegroundLabel: ", self$v.labels[["fg"]], "\n" )
    cat ( "\tbackgroundLabel: ", self$v.labels[["bg"]], "\n" )

    if ( !is.null(self$v.polySize) && !is.null(self$v.numBlobs) )
    {
